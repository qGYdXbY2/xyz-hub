...
////------- 
        ...
        //+ "          ST_SnapToGrid( ST_SetBandNoDataValue( ST_AddBand( ST_MakeEmptyRaster( pxsz, pxsz, st_xmin(tile), st_ymax(tile), ( (st_xmax(tile) - st_xmin(tile))/pxsz::float )::float , ( (st_ymin(tile) - st_ymax(tile))/pxsz::float )::float, 0,0, 4326 ),'1BB' ),0),0.0001,0.0001) as raster, "
        //+ "          ST_SetBandNoDataValue( ST_AddBand( ST_MakeEmptyRaster( (2*pxsz)::integer, (2*pxsz)::integer, round( st_xmin(tile)/dx )*dx, round( st_ymax(tile)/dy )*dy, (dx/pxsz::float)::float , (1.5*-dy/pxsz::float)::float, 0,0, 4326 ),'1BB' ),0) as raster, "
        //+ "          ST_SetBandNoDataValue( ST_AddBand( ST_MakeEmptyRaster( pxsz::integer, pxsz::integer, round( st_xmin(tile)/(dx/pxsz))*(dx/pxsz), round( st_ymax(tile)/(dy/pxsz))*(dy/pxsz), (ceil((st_xmax(tile) - st_xmin(tile))/(dx/pxsz))*(dx/pxsz))/pxsz, (ceil((st_ymin(tile) - st_ymax(tile))/(dy/pxsz))*(dy/pxsz))/pxsz, 0,0, 4326 ),'1BB' ),0) as raster, "
        //+ "          st_clip( ST_SetBandNoDataValue( ST_AddBand( ST_MakeEmptyRaster( nrtiles::integer * pxsz::integer, nrtiles::integer * pxsz::integer, -180, 90, dx, -dy, 0,0, 4326 ),'1BB' ),0),tile,true) as raster, "
        //+ "          ST_SetBandNoDataValue( ST_AddBand( ST_asRaster(tile,pxsz::integer, pxsz::integer ),'1BB' ),0) as raster, "
        + "          level, "
        + "          tile "
        + "         from "
        + "         (  "
...
h3sqlMid2 = " " //+ "st_envelope(st_buffer(ST_MakeEnvelope('-116.19140625', '32.8426736319542982', '-116.015625', '32.9902355596510688', 4326)::geography, (2.5 * edgeLengthM('8')))::geometry) as tile, "
        ...
        + "                   %3$d::integer as pxsz, "
        + "                   (1::bigint << %2$d::integer) as nrtiles, "
        + "                   (360::float / (1::bigint << %2$d::integer)) as dx, " 
        + "                   (180::float / (1::bigint << %2$d::integer)) as dy "
        + "         ) r1 "
        + "        ) r2 "
        + "      ), pxdata as "
        + "      ( "
